...
@router.post("/pin/{pin_id}", response_model=LikeOut, status_code=status.HTTP_201_CREATED)
async def create_like_on_pin(pin_id: int, user_id: user_id, db: db):
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="already_liked")
...
async def delete_like_on_pin(pin_id: int, user_id: user_id, db: db):
    ...
    await db.execute(delete(LikesOrm).where(LikesOrm.pin_id == pin_id, LikesOrm.user_id == user_id))
    await db.commit()
...
@router.post("/comment/{comment_id}", response_model=LikeOut, status_code=status.HTTP_201_CREATED)
...
async def delete_like_on_comment(comment_id: int, user_id: user_id, db: db):
    ...
    await db.execute(
        delete(LikesOrm).where(LikesOrm.comment_id == comment_id, LikesOrm.user_id == user_id)
    )
    await db.commit()
...
async def check_user_like_on_comment(comment_id: int, user_id: user_id, db: db):
    ...
    like = await db.scalar(
        select(LikesOrm).where(LikesOrm.user_id == user_id, LikesOrm.comment_id == comment_id)
    )
    if like:
        return True
    else:
...
async def get_likes_on_comment(comment_id: int, db: db, user_id: user_id, filter: filter):
    comment = await db.scalar(select(CommentsOrm).where(CommentsOrm.id == comment_id))
    if comment is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="comment not found")
